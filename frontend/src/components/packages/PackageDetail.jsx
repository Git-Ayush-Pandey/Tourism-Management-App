...
const PackageDetail = () => {
  const { id } = useParams();
  const { data: pkg, loading, error } = useFetch(() => get(`/packages/${id}`), [id]);

  if (loading) return <Loader />;
  if (error) return <div className="error-text">{error}</div>;

  if (!pkg) return <p className="text-center mt-10 text-gray-500">Package not found.</p>;
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <ul className="space-y-2 mb-6">
            <li><strong>Duration:</strong> {pkg.duration} days</li>
            <li><strong>Price:</strong> ₹{pkg.price}</li>
            <li><strong>Region:</strong> {pkg.region}</li>
          </ul>
